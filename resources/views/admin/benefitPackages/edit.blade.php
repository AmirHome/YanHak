...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.benefitPackage.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.benefit-packages.update", [$benefitPackage->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="title">{{ trans('cruds.benefitPackage.fields.title') }}</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $benefitPackage->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $benefitPackage->description) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('credit_amount') ? 'is-invalid' : '' }}" type="number" name="credit_amount" id="credit_amount" value="{{ old('credit_amount', $benefitPackage->credit_amount) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('benefits') ? 'is-invalid' : '' }}" name="benefits[]" id="benefits" multiple>
                    @foreach($benefits as $id => $benefit)
                        <option value="{{ $id }}" {{ (in_array($id, old('benefits', [])) || $benefitPackage->benefits->contains($id)) ? 'selected' : '' }}>{{ $benefit }}</option>
                    @endforeach
                </select>
