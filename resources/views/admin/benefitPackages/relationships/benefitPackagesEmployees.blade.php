...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-benefitPackagesEmployees">
                <thead>
                    <tr>
                        <th width="10">
                ...
                <tbody>
                    @foreach($employees as $key => $employee)
                        <tr data-entry-id="{{ $employee->id }}">
                            ...
                            <td>
                                @foreach($employee->benfitvariants as $key => $item)
                                    <span class="badge badge-info">{{ $item->name }}</span>
                                @endforeach
                            </td>
                            <td>
                                @foreach($employee->benefit_packages as $key => $item)
...
@endcan
  ...
  let table = $('.datatable-benefitPackagesEmployees:not(.ajaxTable)').DataTable({ buttons: dtButtons })
