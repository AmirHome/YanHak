...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.benefitVariant.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.benefit-variants.update", [$benefitVariant->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.benefitVariant.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $benefitVariant->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $benefitVariant->description) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('credit_amount') ? 'is-invalid' : '' }}" type="number" name="credit_amount" id="credit_amount" value="{{ old('credit_amount', $benefitVariant->credit_amount) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('start_date') ? 'is-invalid' : '' }}" type="text" name="start_date" id="start_date" value="{{ old('start_date', $benefitVariant->start_date) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('end_date') ? 'is-invalid' : '' }}" type="text" name="end_date" id="end_date" value="{{ old('end_date', $benefitVariant->end_date) }}">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\BenefitVariant::SATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('satus') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="satus_{{ $key }}" name="satus" value="{{ $key }}" {{ old('satus', $benefitVariant->satus) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('benefit') ? 'is-invalid' : '' }}" name="benefit_id" id="benefit_id">
                    @foreach($benefits as $id => $entry)
                        <option value="{{ $id }}" {{ (old('benefit_id') ? old('benefit_id') : $benefitVariant->benefit->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
